...
contract Voting is Ownable {
    ...
    mapping(uint256 => bool) private s_nullifierHashes;
    ...
    constructor(address _owner, address _verifier, string memory _question) Ownable(_owner) {
        ...
        // Initialize tree depth if needed, default is usually dynamic or set. 
        // LeanIMT defaults are often fine or need init. 
        // Assuming default initialization is sufficient for dynamic tree.
    }
    ...
    function register(uint256 _commitment) public {
        if (!s_voters[msg.sender]) revert Voting__NotAllowedToVote();
        if (s_hasRegistered[msg.sender]) revert Voting__NotAllowedToVote();
    ...
    function vote(bytes memory _proof, bytes32 _nullifierHash, bytes32 _root, bytes32 _vote, bytes32 _depth) public {
        if (s_nullifierHashes[uint256(_nullifierHash)]) revert Voting__NullifierHashAlreadyUsed(_nullifierHash);
        
        uint256 currentRoot = s_tree.root();
        if (uint256(_root) == 0) revert Voting__EmptyTree();
        if (uint256(_root) != currentRoot) revert Voting__InvalidRoot();
        ...
        s_nullifierHashes[uint256(_nullifierHash)] = true;
        ...
        emit VoteCast(_nullifierHash, msg.sender, uint256(_vote) == 1, block.timestamp, s_yesVotes, s_noVotes);
    }
